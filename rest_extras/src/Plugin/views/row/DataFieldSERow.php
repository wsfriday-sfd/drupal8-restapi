 ...
 * @ViewsRow(
 *   id = "data_field_se",
 *   title = @Translation("Fields SE"),
 *   help = @Translation("Use fields as row data. (Special Edition)"),
 *   display_types = {"data"}
 * )
 */
class DataFieldSERow extends RowPluginBase {
  ...
  /**
   * Stores an array of options to determine if the multi output is used.
   *
   * @var array
   */
  protected $multiOutputOptions = array();
  ...
  public function init(ViewExecutable $view, DisplayPluginBase $display, array &$options = NULL) {
    ...
    if (!empty($this->options['field_options'])) {
      ...
      // Prepare an array of multi output field options.
      $this->multiOutputOptions = static::extractFromOptionsArray('multi_output', $options);
    }
  }
  ...
  public function buildOptionsForm(&$form, FormStateInterface $form_state) {
    ...
    $form['field_options'] = array(
      '#type' => 'table',
      '#header' => array($this->t('Field'), $this->t('Alias'), $this->t('Raw output'), $this->t('Multi output')),
    ...
    if ($fields = $this->view->display_handler->getOption('fields')) {
      foreach ($fields as $id => $field) {
        ...
        $form['field_options'][$id]['multi_output'] = array(
          '#title' => $this->t('Multi output for @id', array('@id' => $id)),
          '#title_display' => 'invisible',
          '#type' => 'checkbox',
          '#default_value' => isset($options[$id]['multi_output']) ? $options[$id]['multi_output'] : '',
        );
      }
    }
  }
  ...
  public function render($row) {
    ...
    foreach ($this->view->field as $id => $field) {
      ...
      // Modification for multi output fields
      if (!empty($this->multiOutputOptions[$id])) {
      	$value = $field->customRender($row);
      }
